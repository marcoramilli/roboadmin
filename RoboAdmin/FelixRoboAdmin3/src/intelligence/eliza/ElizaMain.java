 ...
 * 
 * Modifique esta clase para haciendola un panel para
 * que pueda ser usada tanto como applet como en una
 * ventana normal.
 */
 public class ElizaMain implements IIntellicenceService{
   ...
   Mem mem = new Mem();
    ...
    public ElizaMain(String DictionaryPath){
        this.readScript(true,DictionaryPath);
    System.out.println("Eliza: ELIZA Born ...........");
        ...
        /*
                ...
                response(">> " + input);
                if (finished){
                	JOptionPane.showMessageDialog(ElizaMain.this,reply,"Cerrar",JOptionPane.INFORMATION_MESSAGE);
                	System.exit(0);
                }
                response(reply);
    ...
    String assemble(Decomp d, String reply[], Key gotoKey) {
        ...
        if (EString.match(rule, "goto *", lines)) {
        	...
        	// Este pedazo de c�digo lo agregua para hacer mas interactivo el asunto
        	// esto es si en una regla goto la redirecci�n depende de la entrada entonces 
        	// puede redireccionar a otra clave que se encuentre en la entrada sin estar explicita en el patr�n
        	// esto para que el usuario haga las preguntas y no eliza, entonces si lo que pregunta el usuario no se
        	// encuentra guardado como llave entonces har� el goto hacia xnone
        	if (EString.match(lines[0], "(#)", linesb)){
            	int n = 0;
            	try {
                    n = Integer.parseInt(linesb[0]) - 1;
                } catch (NumberFormatException e) {
                ...
                if (keys.getKey(reply[n]) == null){
                	gotoKey.copy(keys.getKey("xnone"));
                }
                else gotoKey.copy(keys.getKey(reply[n]));
                if (gotoKey.key() != null) return null;
        ...
        switch (event.id) {
            case Event.ACTION_EVENT:
                if (event.target == textfield) {
                    ...
                    response(">> " + input);
                    if (finished){
                    	JOptionPane.showMessageDialog(ElizaMain.this,"Adios",reply,JOptionPane.INFORMATION_MESSAGE);
                    	System.exit(0);
                    }
                    response(reply);
